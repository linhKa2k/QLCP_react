...
const Infor = forwardRef((props: IProps, ref) => {
  const { callback, idEdit, openModal, callbackLoading = () => { return } } = props;
  ...
  useEffect(() => {
    if (idEdit) {
      getPlan(idEdit, callbackGetPlan);
    } else {
      reset({ name: null });
    }
  }, [idEdit]);
  ...
  useImperativeHandle(ref, () => ({
    submitInfor: () => {
      handleSubmit((value) =>
        addOrEditPlan(idEdit ? value : Object.assign(value, {ins_id: user.id}), callback, callbackLoading, idEdit, additionalPlan, watch('parent.id'), department)
      )();
    },
    resetFields: () => {
      reset({});
