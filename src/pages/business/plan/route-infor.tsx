...
import { Button, Col, DatePicker, Input, Popover, Row, Space } from 'antd';
...
const RouteInfor = forwardRef((props: RouteInforProps, ref) => {
  ...
  const addRoute = async (data: any, indexSelect) => {
    ...
    formData.name = `${from.length ? from : formData?.from_province?.name} - ${
      to.length ? to : formData?.to_province?.name
    }`;
    // if (isAir) {
